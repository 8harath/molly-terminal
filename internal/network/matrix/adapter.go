...
// from the keyring (service "molly-matrix") at Connect time.
...
// MOLLY_MATRIX_PASSWORD on first run) and starts the /sync loop.
...
// passwordLogin performs an m.login.password flow using MOLLY_MATRIX_PASSWORD
...
func (a *Adapter) passwordLogin(ctx context.Context, client *mautrix.Client) error {
	password := os.Getenv("MOLLY_MATRIX_PASSWORD")
	if password == "" {
		return fmt.Errorf("matrix: no stored token and MOLLY_MATRIX_PASSWORD is unset")
	}
	resp, err := client.Login(ctx, &mautrix.ReqLogin{
		Type:                     mautrix.AuthTypePassword,
		Identifier:               mautrix.UserIdentifier{Type: mautrix.IdentifierTypeUser, User: localpart(a.userID)},
		Password:                 password,
		InitialDeviceDisplayName: "molly-terminal",

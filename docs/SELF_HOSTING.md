...
By default molly talks to the public infrastructure run by ploglabs
(`molly.ploglabs.com`) and uses a pre-registered Discord application. This
guide explains how to point molly at your own infrastructure instead.

## Architecture

molly is a **client**. At runtime it talks to three things:

| Concern | Component | Default | Config key |
|---|---|---|---|
| Receiving messages in realtime | WebSocket **relay** | `wss://molly.ploglabs.com:8443/ws` | `server.websocket_url` |
| Loading message history | relay **REST** API | `https://molly.ploglabs.com:8443` | `server.relay_url` |
...
> that speaks molly's protocol. If you only want to use your own Discord app
...
3. Note the **Client ID**. molly uses PKCE (public client), so you do **not**
   need a client secret for the default flow. Set one only for a confidential
   deployment.

Point molly at your app:
...
```bash
export MOLLY_DISCORD_CLIENT_ID=YOUR_CLIENT_ID
export MOLLY_DISCORD_REDIRECT_URL=http://127.0.0.1:53682/callback
# optional, confidential deployments only:
export MOLLY_DISCORD_CLIENT_SECRET=...
```
...
or `export MOLLY_WEBHOOK_URL=...`.

## 3. Point molly at your own relay

If you run a molly-compatible relay, override both the WebSocket and REST
endpoints:
...
History fetching is disabled if `relay_url` is empty — molly still works as a
...
| `MOLLY_API_KEY` | `server.api_key` (relay auth, if your relay requires it) |
| `MOLLY_WEB_SETUP_URL` | `server.web_setup_url` (web onboarding wizard) |
...
| `MOLLY_GUILD_ID` | `general.guild_id` |
| `MOLLY_BOT_CLIENT_ID` | `server.bot_client_id` |
...
```bash
make build
./bin/molly --config /path/to/your/config.toml
```

On first run molly opens your browser for the OAuth flow against **your**

...
This document explains how molly moves data around — what stays on your
machine, what is sent to Discord directly, and what passes through the relay.
It exists to answer a fair and recurring question: **why does molly use a
...
molly **does not connect to Discord's gateway as your user account**, and that
...
suspension. molly will not ship a self-bot.

So molly is built around a **server-side bot + webhooks**:

- A Discord **bot** (the relay) holds the gateway connection and fans out
  realtime events to connected molly clients over a WebSocket.
...
and metadata pass through `molly.ploglabs.com`. That is the cost of the
ToS-safe, zero-setup design. The escape hatch is self-hosting the relay (or
using a send-only webhook with no relay at all).

## What goes where

At runtime molly talks to two places: **Discord directly**, and **the relay**.
...
- **Message content and metadata** for messages you send and for the channels
  ...
  `molly.ploglabs.com`. Treat this the way you would any hosted chat bridge.

The public relay
([`ploglabs/molly-discord-relay`](https://github.com/ploglabs/molly-discord-relay))
...
this is unacceptable for your threat model, self-host the relay or run molly
send-only (see below).
...
- **Web browser setup:** molly opens
  `https://molly.ploglabs.com/terminal-setup#token=<access_token>` so the web
...
If you do not want your token to reach `molly.ploglabs.com`, use **terminal
setup**, or point `server.web_setup_url` at your own host
(`MOLLY_WEB_SETUP_URL`). Self-hosters should override it.

## Reducing or removing reliance on the relay

molly is a client; every endpoint is configurable.

- **Self-host the relay** — point `server.websocket_url` and `server.relay_url`
  at your own molly-compatible relay. See [`SELF_HOSTING.md`](SELF_HOSTING.md).
- **Send-only, no relay** — leave `server.relay_url` empty and set
  `server.webhook_url`. molly sends via a Discord webhook directly; history and
